...
load_dotenv()
COLLECTION: str = os.getenv("COLLECTION_SCRAPPER", "scrapper")
...
def parse_questions(html: str) -> List[Question]:
    ...
    for question_div in soup.select(".s-post-summary"):
        ...
        tags: List[str] = [t.text for t in question_div.select(".s-post-summary--meta-tags a")]
...
async def fetch_page(session: ClientSession, url: str) -> List[Question]:
    async with session.get(url) as response:
        html: str = await response.text()
        return parse_questions(html)

async def scrape_async(pages: int = 1) -> List[Question]:
    base_url: str = "https://stackoverflow.com/questions?tab=Newest&pagesize=50&page="
    urls: List[str] = [f"{base_url}{i}" for i in range(1, pages + 1)]

    async with aiohttp.ClientSession() as session:
        tasks = [fetch_page(session, url) for url in urls]
        results: List[List[Question]] = await asyncio.gather(*tasks)
    return [q for page in results for q in page]

def run_scraper(pages: int = 5, append: bool = False) -> None:
    start: float = time.time()
    questions: List[Question] = asyncio.run(scrape_async(pages))
...
if __name__ == "__main__":
    run_scraper(pages=5, append=True)
